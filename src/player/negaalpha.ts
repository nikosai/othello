...
export class NegaAlphaAIPlayer extends Player {
  ...
  onMyTurn(board: Board, onPut: (x: number, y: number) => Board | null, enemySkipped?: boolean) {
    const ret = this.negaAlpha(board, this.depth,
      { v: -Infinity, c: null },
      { v: Infinity, c: null }).c;
    if (!ret) {
  ...
  negaAlpha(node: Board, depth: number, alpha: AlphaBetaRet, beta: AlphaBetaRet):AlphaBetaRet {
    ...
    for (const c of children) {
      ...
      const ret = this.negaAlpha(child, depth - 1, { v: -beta.v, c: beta.c }, { v: -alpha.v, c: alpha.c });
      if (alpha.v < ret.v) {
        alpha = { v: ret.v, c: c };
      }
